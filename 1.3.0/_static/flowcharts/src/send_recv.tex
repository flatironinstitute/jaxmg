...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[stage,minimum width=5.1cm,text width=4.35cm,fill=gpu0!20]
    (source4) at (-3,8.45) {Source \\matrix rectangle};

  \node[slot]        (send4) at (-3,4.45) {Send slot};
  \node[receive]     (recv4) at (3,4.45)     {Receive slot};
  \node[destination] (dest4) at (3,8.45)  {Destination \\matrix rectangle};

  \draw[flow] (source4) --
    node[transition,midway] {pack}
    (send4);

  \draw[flow] (send4) -- (recv4);
  \draw[flow] (recv4) --
  node[transition,midway] {unpack}
  (dest4);
\end{tikzpicture}
