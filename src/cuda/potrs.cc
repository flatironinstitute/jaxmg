...
absl::Status XlaCommPotrsMgNativePlanImpl(
  ...
  if (current_device == 0) {
    ...
    for (int dev = 0; dev < num_ranks; ++dev) {
      device_list[dev] = dev;
      a_ptrs[dev] = fused_potrs_state.a[dev];
      b_ptrs[dev] = fused_potrs_state.b[dev];
    }
  ...
  absl::StatusOr<void*> solver_work = AllocateFfiScratch(
      scratch, sizeof(DataType) * fused_potrs_state.lwork[current_device],
  ...
  if (current_device == 0) {
    ...
    for (int dev = 0; dev < num_ranks; ++dev) {
      work_ptrs[dev] = fused_potrs_state.work[dev];
    }
    solver_status = cusolverMgPotrf(
        cusolver, CUBLAS_FILL_MODE_LOWER, n, a_ptrs.data(), ia, ja, descr_a,
        compute_type, work_ptrs.data(), fused_potrs_state.lwork[0], &info);
    JAXMG_RETURN_IF_CUDA_ERROR(cudaStreamSynchronize(cuda_stream));
    timer.Mark("potrf");
    for (int dev = 0; dev < num_ranks; ++dev) {
    ...
    if (solver_status == CUSOLVER_STATUS_SUCCESS) {
      solver_status = cusolverMgPotrs(
          ...
          work_ptrs.data(), fused_potrs_state.lwork[0], &info);
      JAXMG_RETURN_IF_CUDA_ERROR(cudaStreamSynchronize(cuda_stream));
      timer.Mark("potrs");
      for (int dev = 0; dev < num_ranks; ++dev) {
        fused_potrs_state.solver_status[dev] =
  ...
  const int32_t status_value = fused_potrs_state.solver_status[current_device];
  if (status_value == 0) {
