...
#define SOLVER_DISPATCH_IMPL(impl, ...)            \
    ...
    default:                                       \
                               ...
                               ffi::Result<ffi::Buffer<ffi::S32>> status)
        {
            ...
            const std::string shmA_name = absl::StrFormat("/jaxmg_syevd_no_V_shmA_%d", getppid());
            const std::string shmev_name = absl::StrFormat("/jaxmg_syevd_no_V_shmev_%d", getppid());
            const std::string shmwork_name = absl::StrFormat("/jaxmg_syevd_no_V_shmwork_%d", getppid());
            const std::string shmcsh_name = absl::StrFormat("/jaxmg_syevd_no_V_shmcsh_%d", getppid());
            const std::string shmlwork_name = absl::StrFormat("/jaxmg_syevd_no_V_shmlwork_%d", getppid());
                                   ...
                                   ffi::Result<ffi::Buffer<ffi::S32>> status)
        {
            ...
            SOLVER_DISPATCH_IMPL(SyevdMgImpl, N, batch_a, stream, scratch, a, tile_size, eigenvalues, status);
