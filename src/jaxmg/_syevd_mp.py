...
def syevd_mp(
    ...
    Multi-node ``syevd_mp`` uses the XLA-owned NCCL communicator attached to
    the JAX mesh. The JAX distributed setup remains user-owned; JAXMg validates
    the resulting mesh/rank mapping before entering the native cuSOLVERMp call.
