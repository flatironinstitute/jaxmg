...
def test_detect_runtime_mode_explicit_mpmd_uses_explicit_devices(monkeypatch):
    monkeypatch.setenv("JAXMG_EXECUTION_MODE", "MPMD")
    monkeypatch.setenv("JAXMG_NUMBER_OF_DEVICES", "8")
    monkeypatch.setattr(setup.jax.distributed, "is_initialized", lambda: True)
    monkeypatch.setattr(setup.jax, "local_device_count", lambda: 1)
...
def test_detect_runtime_mode_distributed_node_spmd(monkeypatch):
    monkeypatch.delenv("JAXMG_NUMBER_OF_DEVICES", raising=False)
    monkeypatch.delenv("JAXMG_EXECUTION_MODE", raising=False)
    monkeypatch.setattr(setup.jax.distributed, "is_initialized", lambda: True)
    monkeypatch.setattr(setup.jax, "local_device_count", lambda: 4)
    monkeypatch.setattr(setup.jax, "device_count", lambda: 8)

    assert setup._detect_runtime_mode() == ("SPMD", 4)


def test_detect_runtime_mode_explicit_node_spmd(monkeypatch):
    monkeypatch.setenv("JAXMG_EXECUTION_MODE", "SPMD")
    monkeypatch.setenv("JAXMG_NUMBER_OF_DEVICES", "4")
    monkeypatch.setattr(setup.jax.distributed, "is_initialized", lambda: True)
    monkeypatch.setattr(setup.jax, "local_device_count", lambda: 1)
