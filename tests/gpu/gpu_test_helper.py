...
def run_gpu_test(
    ...
    *,
    ...
    with the abstract mesh inside the caller's jit (``abstract_mesh``), or a
    check that an unsupported device order is rejected (``invalid_order``).
    """
    if interface not in ("public", "context", "abstract_mesh", "invalid_order"):
